...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
	...
	public boolean update(Message message) throws NotSavedSubEntityException{
		...
		String SQL_UPDATE_MESSAGE = ("UPDATE chat.message SET " +
				"author = " + (message.getAuthor().getId()) +
				", chatroom = " + (message.getChatroom().getId()) +
				", text = \'" + (message.getText()) +
				"\', messageDateTime = \'" + (Timestamp.valueOf(message.getMessageDateTime())) +
				"\' WHERE chat.message.id = " + (message.getId()));
		try (Connection connection = dataSource.getConnection()) {
			Statement statement = connection.createStatement();
			statement.executeUpdate(SQL_UPDATE_MESSAGE);
